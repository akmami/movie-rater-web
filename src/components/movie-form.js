...
function MovieForm(props) {
    ...
    const updateClicked = () => {
        API.updateMovie(props.movie.id, {title: title, description: description}, (token['mr-token'])['token'])
        .then( response => props.updateMovie(response) ) 
        .catch( error => console.log(error) );
    }

    const createClicked = () => {
        API.createMovie({title: title, description: description}, (token['mr-token'])['token'])
